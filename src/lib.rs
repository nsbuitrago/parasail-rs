...
impl AlignResult {
    ...
    pub fn get_trace_ins_table(&self) -> Result<*mut i32, std::io::Error> {
        if self.is_trace() {
            unsafe { Ok(parasail_result_get_trace_ins_table(self.inner)) }
        } else {
            Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                "Trace insertion table is not available without setting use_trace",
            ))
        }
    }
